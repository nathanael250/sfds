...
def index():
    ...
    pending_verifications = StockRequest.query.filter_by(
...
def stock_dashboard():
    ...
    total_value = (
        db.session.query(func.sum(Product.price_per_unit * Stock.quantity))
        .join(Stock, Stock.product_id == Product.id)
        .scalar()
        or 0
    )
    total_value = (
        db.session.query(func.sum(Product.price_per_unit * Product.quantity)).scalar()
        or 0
    )
    pending_requests = StockRequest.query.filter_by(status="approved_by_sao").count()
    pending_verifications = StockRequest.query.filter_by(
    ...
    recent_requests = (
        StockRequest.query.filter_by(status="approved_by_sao")
    ...
    recent_verifications = (
        StockRequest.query.filter_by(status="approved_by_md")
        .order_by(StockRequest.created_at.desc())
        .limit(5)
        .all()
    )
...
def view_stock_receipt(request_id):
    ...
    try:
        ...
        if not upload_folder:
            ...
            return (
                jsonify(
                    {"error": "Server configuration error: No upload folder defined"}
                ),
                500,
            )
        ...
        return jsonify(
            {"type": file_type, "data": base64.b64encode(file_data).decode("utf-8")}
        )
    except Exception as e:
...
# @bp.route("/pending_verifications/<int:request_id>/view")
# @login_required
# @requires_roles("md")
# def view_receipt(request_id):
#     """View a specific receipt for verification."""
#     # Get the stock request
#     request = StockRequest.query.get_or_404(request_id)
    
#     # Check if the request is in the correct status for verification
#     if request.status != "approved_by_md":
#         flash("This request is not ready for receipt verification", "error")
#         return redirect(url_for("md.stock_verifications"))
    
#     return render_template("md/view_receipt.html", request=request)

@bp.route("/receipt/<int:request_id>/view")
@login_required
@requires_roles("md")
def view_receipt(request_id):
    """View receipt page."""
    stock_request = StockRequest.query.get_or_404(request_id)
    return render_template("md/view_receipt.html", request=stock_request)


@bp.route("/receipt/<int:request_id>/file")
@login_required
@requires_roles("md")
def serve_receipt_file(request_id):
    """Serve the actual receipt file."""
    stock_request = StockRequest.query.get_or_404(request_id)
    
    if not stock_request.receipt_path:
        flash("No receipt found for this request", "error")
        return redirect(url_for('md.view_receipt', request_id=request_id))
    ...
    try:
        ...
        file_name = os.path.basename(stock_request.receipt_path)
        ...
        flash(f"Error serving receipt file: {str(e)}", "error")
        return redirect(url_for('md.view_receipt', request_id=request_id))
...
def export_transaction_report():
    ...
    if export_format == "pdf":
        ...
        # Create table data
        table_data = [
            ["Date", "Agrodealer", "Product", "Quantity", "Total Amount", "Citizen"]
        ]
        ...
        output.seek(0)
        filename = (
            f"transactions_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        )
        ...
        return send_file(
            output, download_name=filename, as_attachment=True, mimetype="text/csv"
        )
...
def stock_health():
    ...
    for agrodealer in agrodealers:
        ...
        total_value = (
            db.session.query(func.sum(Product.price_per_unit * Stock.quantity))
            .join(Stock, Stock.product_id == Product.id)
            .filter(Stock.agrodealer_id == agrodealer.id)
            .scalar()
            or 0
        )
        ...
        recent_transactions = (
            Transaction.query.filter_by(sold_by=agrodealer.id)
            .order_by(Transaction.created_at.desc())
            .limit(5)
            .all()
        )
